        ...
        .then((group) => {
          if (group) {
            setGroupList((prevGroups) => [
              ...prevGroups,
              { id: group.id, name: group.name, lastMessage: group.lastMessage, unreadCount: group.unreadCount }
            ]);
          } else {
            console.warn(`Grup bulunamadı: ${groupID}`);
          }
        })
        .catch((error) => {
          console.error(`Grup yüklenirken hata oluştu: ${error}`);
        });
    });
  }, [groupList]);
